...
int16_t Hx_offset = 35, Hy_offset = -35, Hz_offset = -10;
...
void  HMC58X3_newValues(int16_t x,int16_t y,int16_t z)
{
	...
	HMC5883_FIFO[1][9]=y;
	HMC5883_FIFO[2][9]=z;
	HMC5883_FIFO[0][10]=x - Hx_offset;
	HMC5883_FIFO[1][10]=y - Hy_offset;
	HMC5883_FIFO[2][10]=z - Hz_offset;

	/*sum=0;
	for(i=0;i<10;i++){	//取数组内的值进行求和再取平均
   		sum+=HMC5883_FIFO[0][i];
	}
	HMC5883_FIFO[0][10]=sum/10 - Hx_offset;	//将平均值更新

	sum=0;
	for(i=0;i<10;i++){
   		sum+=HMC5883_FIFO[1][i];
	}
	HMC5883_FIFO[1][10]=sum/10 - Hy_offset;

	sum=0;
	for(i=0;i<10;i++){
   		sum+=HMC5883_FIFO[2][i];
	}
	HMC5883_FIFO[2][10]=sum/10 - Hz_offset;*/
} //HMC58X3_newValues
